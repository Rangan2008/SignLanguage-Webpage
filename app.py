...
class SignLanguageApp:
    def __init__(self):
        ...
        # Create white background image (load once, reuse)
        self.white = np.ones((400, 400, 3), dtype=np.uint8) * 255
        cv2.imwrite("white.jpg", self.white)
        
        # Performance optimization variables
        self.frame_skip_counter = 0
        self.frame_skip_rate = 3  # Process every 3rd frame only
        self.last_prediction_time = 0
        self.min_prediction_interval = 0.5  # Minimum 0.5 seconds between predictions
        self.cached_white_img = self.white.copy()  # Cache white image
    ...
    def predict_gesture(self, frame):
        try:
            if self.model is None:
                return None, self.cached_white_img.copy()
            
            # Performance optimization: Skip frames
            self.frame_skip_counter += 1
            if self.frame_skip_counter < self.frame_skip_rate:
                return self.current_symbol, self.cached_white_img.copy()
            
            self.frame_skip_counter = 0  # Reset counter
            
            # Time-based throttling for predictions
            ...
            if width > 480:  # Reduced from 640 for better performance
                scale = 480 / width
                new_width = 480
            ...
            bonestructure_img = self.cached_white_img.copy()  # Use cached image
            
            if results.multi_hand_landmarks:
                self.last_prediction_time = current_time  # Update prediction time
                ...
                # Optimized drawing - fewer operations
                for connection in self.mp_hands.HAND_CONNECTIONS:
                    ...
                    cv2.line(bonestructure_img, (x1s, y1s), (x2s, y2s), (0, 255, 0), 2)  # Thinner lines
                
                for i in range(21):
                    px = int(np.clip(lmList[i][0] - x1 + os, 0, 399))
                    py = int(np.clip(lmList[i][1] - y1 + os1, 0, 399))
                    cv2.circle(bonestructure_img, (px, py), 1, (0, 0, 255), 1)  # Smaller circles
    ...
    def predict(self, test_image):
        ...
        # Performance optimization: Use cached results if image hasn't changed much
        current_time = time.time()
        if (hasattr(self, 'last_cnn_prediction_time') and 
            current_time - self.last_cnn_prediction_time < 0.3 and  # Cache for 300ms
            self.current_symbol and self.current_symbol != "Empty"):
            return self.current_symbol
            
        white = test_image
        white = white.reshape(1, 400, 400, 3)
        
        # CNN inference - most expensive operation
        prob = np.array(self.model.predict(white)[0], dtype='float32')
        self.last_cnn_prediction_time = current_time  # Update cache time
